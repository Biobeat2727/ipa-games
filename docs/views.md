...
- Sandbox/design preview: `/preview` route → `src/routes/preview/TapPreview.tsx` — standalone page for iterating on the glass/tap visuals without needing a live game session. Not linked from any real navigation; safe to leave in place or delete later. `/preview?cats=TV Dads,Leftovers,…` swaps in real category names — check a week's names at 375px before the night.
- Sign text fitting (`TapHeader`): a 375px phone column fits ~6 uppercase characters per line. Multi-word names wrap between words; a single long word is measured (hidden probe span) and the sign's font shrinks just enough to fit it on one line, down to 0.7×. Words of 9+ letters carry a soft hyphen at the midpoint and break there ("CONSPI-RACIES") when even the floor can't fit them, sized for the longer half. Browser auto-hyphenation is deliberately not used (needs a dictionary the phone may not have).
